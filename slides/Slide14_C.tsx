...
const Card: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode; className?: string }> = ({ icon, title, children, className }) => (
    <div className={`bg-white rounded-xl shadow-lg border border-slate-200 p-6 flex flex-col ${className}`}>
        <h3 className="text-lg font-bold text-[#1a2947] mb-4 flex items-center gap-3">{icon}{title}</h3>
...
const TimelineItem: React.FC<{ children: React.ReactNode; isLast?: boolean;}> = ({ children, isLast = false }) => (
    <div className="flex gap-3">
        ...
        <div className="pb-4 text-sm flex-grow">{children}</div>
    </div>
);
        ...
        <SlideWrapper className="p-12">
            ...
            >
                ...
                <div className="border-b border-slate-200 pb-4 mb-6">
                    <h2 className="text-3xl font-bold text-[#1a2947]">Caso 4588: Compañía XYZ S.L.</h2>
                ...
                <div className="grid grid-cols-2 grid-rows-2 gap-6 h-[450px]">
                    <Card icon={<Euro className="text-[#4a9eff]"/>} title="Análisis Financiero y Pricing IA">
                         <div className="bg-slate-100 p-4 rounded-lg">
                            <p className="font-bold text-cyan-800 text-sm">Rango de Honorarios Recomendado IA:</p>
                            <p className="text-3xl font-bold text-[#1a2947] mt-1">€6,000 — €8,500</p>
                        </div>
                         <div className="mt-4 flex items-center gap-4">
                            <label className="font-semibold text-sm">Propuesta:</label>
                            <input type="text" value="€7,200" readOnly className="flex-grow bg-white border-2 border-slate-200 rounded-md p-2 text-lg font-bold text-[#1a2947] text-center" />
                            <button className="bg-[#4a9eff] text-white font-bold py-2.5 px-4 rounded-md text-sm hover:bg-[#3b7dd6] transition-colors">Enviar</button>
                        </div>
                    </Card>
                    ...
                    <Card icon={<Clock className="text-[#4a9eff]"/>} title="Línea de Tiempo">
                        <div className="relative">
                            ...
                            <TimelineItem><p><strong className="font-semibold">Hace 2 días:</strong> Lead recibido (WhatsApp)</p></TimelineItem>
                        </div>
                    </Card>
                    ...
                    <Card icon={<FileText className="text-[#4a9eff]"/>} title="Documentos">
                        <div className="flex items-center justify-between bg-slate-100 p-3 rounded-md">
                            <p className="text-base font-semibold text-slate-800">Contrato_V1.pdf</p>
                            <span className="text-sm font-semibold text-[#065f46] bg-[#d1fae5] px-2 py-0.5 rounded">Analizado</span>
                        </div>
                    </Card>
                    
                    <Card icon={<Check className="text-[#4a9eff]"/>} title="Próximas Tareas IA">
                        <ul className="space-y-2 text-base text-slate-700">
                           <li className="flex items-center gap-2 p-2 bg-slate-100 rounded-md"><input type="checkbox" className="w-5 h-5 accent-[#4a9eff]" /> Revisar y confirmar honorarios. <span className="ml-auto text-xs font-bold bg-red-500/10 text-red-600 px-2 py-0.5 rounded">ALTA</span></li>
                           <li className="flex items-center gap-2 p-2 bg-slate-100 rounded-md"><input type="checkbox" className="w-5 h-5 accent-[#4a9eff]" /> Enviar propuesta al cliente. <span className="ml-auto text-xs font-bold bg-red-500/10 text-red-600 px-2 py-0.5 rounded">ALTA</span></li>
                        </ul>
                    </Card>
                </div>
            </motion.div>
