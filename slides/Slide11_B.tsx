                ...
                <div className="w-1/4 p-4 flex flex-col">
                    ...
                    <div className="space-y-3">
                         {[{n:'Contrato_Principal.pdf', a:true},{n:'Anexo II P.I.pdf', a:false},{n:'Normativa_Aplicable.pdf', a:false}].map(d=>(
                            <div key={d.n} className={`p-3 rounded-md flex items-center gap-3 border transition-colors ${d.a || (d.n.includes('Anexo') && highlightedClause === 'c2') ? 'bg-cyan-500/10 border-cyan-500/50' : 'bg-white border-slate-200'}`}>
                                <FileText className={`w-8 h-8 flex-shrink-0 ${d.a || (d.n.includes('Anexo') && highlightedClause === 'c2') ? 'text-cyan-500' : 'text-slate-400'}`}/>
                                <p className={`font-medium text-xl ${d.a || (d.n.includes('Anexo') && highlightedClause === 'c2') ? 'text-slate-800' : 'text-slate-500'}`}>{d.n}</p>
                            </div>
                        ))}
                    </div>
                </div>
                                    ...
                                    {msg.type === 'processing' ? (
                                        ...
                                        <div className={`p-3 rounded-lg max-w-lg text-xl ${msg.from === 'user' ? 'bg-cyan-600 text-white rounded-br-none' : 'bg-white text-slate-700 rounded-bl-none border border-slate-200'}`}>
                                            ...
                                            {msg.citations && msg.citations.map(c => (
                                                <p key={c.id} className={`mt-2 p-2 rounded cursor-pointer transition-colors border-l-4 ${highlightedClause === c.id ? 'bg-yellow-100 border-yellow-400' : 'bg-slate-100 border-slate-300'}`}>{c.text}</p>
                                            ))}
                                        </div>
                                    )}
                                </div>
